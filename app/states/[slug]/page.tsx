...
export default function StatePage({ params }: PageProps) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 sm:py-16">
      ...
      {/* Hero */}
      <section>
        <div className="flex items-center gap-2 text-sm text-maroon-800">
      ...
      <section className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div className="flex flex-col items-center rounded-xl border border-ivory-200 bg-white p-5 text-center">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-ivory-100">
        ...
        <div className="flex flex-col items-center rounded-xl border border-ivory-200 bg-white p-5 text-center">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-ivory-100">
            <Clock className="h-5 w-5 text-maroon-800" />
          </div>
      ...
      {/* State-specific info */}
      <section className="mt-10 rounded-xl border border-ivory-200 bg-white p-6">
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row">
          <a
            ...
            className="inline-flex items-center gap-1.5 text-sm font-medium text-maroon-800 hover:text-maroon-900"
          >
        ...
        <ol className="mt-4 space-y-4">
          {[
            ...
            <li key={i} className="flex gap-3">
              <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-maroon-800 text-sm font-bold text-white">
                {i + 1}
              </span>
      ...
      <section className="mt-10">
        ...
        <div className="mt-4 space-y-4">
          {faqItems.map((item, i) => (
            <div key={i} className="rounded-xl border border-ivory-200 bg-white p-5">
              <h3 className="font-serif font-semibold text-gray-900">{item.question}</h3>
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {otherStates.map((s) => (
            <Link
              key={s.slug}
              href={`/states/${s.slug}`}
              className="flex items-center justify-between rounded-lg border border-ivory-200 bg-white px-4 py-3 text-sm transition-all hover:border-maroon-300 hover:shadow-sm"
            >
              <div className="flex items-center gap-2">
                <span className="font-bold text-maroon-800">{s.abbreviation}</span>
                <span className="text-gray-700">{s.name}</span>
              </div>
              <span className="rounded-full bg-ivory-100 px-2 py-0.5 text-xs font-semibold text-gray-600">
                {s.passingScore}
              </span>
            </Link>
          ))}
