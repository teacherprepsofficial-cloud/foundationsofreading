...
export async function POST(request: NextRequest) {
  try {
    const { email, source, pdfSlug, emailSubject, emailHeading, emailBody } = await request.json()
    if (!email || !source) {
...
<body style="margin:0;padding:0;background:#f4f0eb;font-family:Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:24px 16px;">
    ...
    <div style="background:#7c1c2e;border-radius:8px 8px 0 0;padding:32px;text-align:center;">
      ...
      <h1 style="margin:0;font-size:22px;color:white;font-family:Georgia,serif;line-height:1.3;">${emailHeading || 'Your Free Guide is Ready to Download'}</h1>
    </div>
    ...
    <div style="background:white;padding:36px 32px;border-radius:0 0 8px 8px;">
      <p style="font-size:15px;line-height:1.7;color:#333;margin:0 0 20px;">
        ${emailBody || 'Here\'s your free guide — download it using the button below.'}
      </p>
      ...
      <div style="text-align:center;margin:28px 0;">
        ...
        <p style="margin:10px 0 0;color:#9ca3af;font-size:12px;">PDF · Free Download</p>
      </div>
    ...
    await resend.emails.send({
      ...
      to: email,
      subject: emailSubject || 'Your Free Study Resource',
      html,
    })
