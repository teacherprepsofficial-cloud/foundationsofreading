...
export async function GET() {
  try {
    ...
    p1.drawText('MISSISSIPPI — FOUNDATIONS OF READING', { x: MARGIN, y: PAGE_H - 25, size: 8, font: bold, color: rgb(0.91, 0.71, 0.74) })
    p1.drawText('FORT 890 · Test Guide', { x: MARGIN, y: PAGE_H - 46, size: 17, font: bold, color: WHITE })
    ...
    y = sectionBar(p1, y, 'Mississippi — Test at a Glance')

    const glanceCol1W = 96
    const glanceRows: [string, string][] = [
      ['Test Code',     '890 — Foundations of Reading (Mississippi)'],
      ...
      ['Fee',           '$139'],
      ['Passing Score', '233 (Mississippi)'],
      ['Score Scale',   '100–300'],
      ['Retake Policy', '30-day minimum wait · No limit on number of attempts'],
      ['Registration',  'nestest.com/state/ms — Mississippi NES testing portal'],
    ]
    ...
    y2R = sectionBar(p2, y2R, 'Test Day Tips — Mississippi', COL_W, COL2_X)
    const tips = [
      'Mississippi passing score is 233 — lower than the 240 national benchmark.',
      'Register through nestest.com/state/ms — the Mississippi NES portal.',
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="Mississippi-FORT-890-Test-Guide.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('Mississippi Test Guide PDF error:', err)
