...
export async function GET() {
  try {
    ...
    p1.drawText('ARKANSAS — FOUNDATIONS OF READING', { x: MARGIN, y: PAGE_H - 25, size: 8, font: bold, color: rgb(0.91, 0.71, 0.74) })
    p1.drawText('FORT 890 · Quick Reference Guide', { x: MARGIN, y: PAGE_H - 46, size: 17, font: bold, color: WHITE })
    ...
    // ── Alabama at a Glance ─────────────────────────────────────────────────
    y = sectionBar(p1, y, 'Arkansas — Test at a Glance')

    const glanceCol1W = 96
    const glanceRows: [string, string][] = [
      ['Test Code',     '890 — Foundations of Reading (Arkansas uses NES in Arkansas)'],
      ...
      ['Fee',           '$139'],
      ['Passing Score', '233 (Arkansas) · National benchmark is 240'],
      ['Score Scale',   '100–300'],
      ['Retake Policy', '30-day minimum wait · No limit on number of attempts'],
      ['Registration',  'National Evaluation Series in Arkansas page (not the NES 190 page)'],
    ]
    ...
    y2R = sectionBar(p2, y2R, 'Test Day Tips — Arkansas', COL_W, COL2_X)
    const tips = [
      'Arkansas passing score is 233 — lower than the 240 national benchmark.',
      'Register through the NES in Arkansas page, not the standard NES 190 page.',
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="Arkansas-FORT-890-Quick-Reference-Guide.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('Arkansas PDF error:', err)
