...
export async function GET() {
  try {
    ...
    p1.drawText('OHIO / OAE — FOUNDATIONS OF READING', { x: MARGIN, y: PAGE_H - 25, size: 8, font: bold, color: rgb(0.91, 0.71, 0.74) })
    p1.drawText('OAE 190 · Study Sheet', { x: MARGIN, y: PAGE_H - 46, size: 17, font: bold, color: WHITE })
    ...
    // ── Ohio / OAE at a Glance ──────────────────────────────────────────────
    y = sectionBar(p1, y, 'Ohio / OAE — Test at a Glance')

    const glanceCol1W = 96
    const glanceRows: [string, string][] = [
      ['Test Code',     '190 — Foundations of Reading (OAE program in Ohio)'],
      ...
      ['Fee',           '$139'],
      ['Passing Score', '220 (Ohio)'],
      ['Score Scale',   '100–300'],
      ['Retake Policy', '30-day minimum wait · No limit on number of attempts'],
      ['Registration',  'oh.nesinc.com — Ohio Assessments for Educators portal'],
    ]
    ...
    y2R = sectionBar(p2, y2R, 'Test Day Tips — Ohio / OAE', COL_W, COL2_X)
    const tips = [
      'Ohio passing score is 220 — lower than the 240 national benchmark.',
      'Register through oh.nesinc.com — the OAE portal for Ohio.',
      'Test code is 190 in Ohio. Same content as the 890, different registration portal.',
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="OAE-190-Foundations-of-Reading-Study-Sheet.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('OAE Study Sheet PDF error:', err)
