...
export async function GET() {
  try {
    ...
    p1.drawText('WISCONSIN — FOUNDATIONS OF READING', { x: MARGIN, y: PAGE_H - 25, size: 8, font: bold, color: rgb(0.91, 0.71, 0.74) })
    p1.drawText('FORT 890 · Exam Guide', { x: MARGIN, y: PAGE_H - 46, size: 17, font: bold, color: WHITE })
    ...
    y = sectionBar(p1, y, 'Wisconsin — Test at a Glance')

    const glanceCol1W = 96
    const glanceRows: [string, string][] = [
      ['Test Code',     '890 — Foundations of Reading (Wisconsin accepts 090, 190, and 890 scores)'],
      ...
      ['Fee',           '$139'],
      ['Passing Score', '240 (Wisconsin)'],
      ['Score Scale',   '100–300'],
      ['Retake Policy', '30-day minimum wait · No limit on number of attempts'],
      ['Registration',  'wi.nesinc.com — Wisconsin educator testing portal'],
    ]
    ...
    y2R = sectionBar(p2, y2R, 'Test Day Tips — Wisconsin', COL_W, COL2_X)
    const tips = [
      'Wisconsin passing score is 240 — the national benchmark.',
      'Register through wi.nesinc.com — the Wisconsin testing portal.',
      'Test code is 890. Wisconsin also accepts 090 and 190 scores.',
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="Wisconsin-FORT-890-Exam-Guide.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('Wisconsin PDF error:', err)
