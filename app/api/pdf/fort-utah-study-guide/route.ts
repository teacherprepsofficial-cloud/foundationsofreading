...
export async function GET() {
  try {
    ...
    p1.drawText('UTAH — FOUNDATIONS OF READING', { x: MARGIN, y: PAGE_H - 25, size: 8, font: bold, color: rgb(0.91, 0.71, 0.74) })
    p1.drawText('FORT 890 · Study Guide', { x: MARGIN, y: PAGE_H - 46, size: 17, font: bold, color: WHITE })
    ...
    // ── Utah at a Glance ────────────────────────────────────────────────────
    y = sectionBar(p1, y, 'Utah — Test at a Glance')

    const glanceCol1W = 96
    const glanceRows: [string, string][] = [
      ['Test Code',     '890 — Foundations of Reading (Utah)'],
      ...
      ['Fee',           '$139'],
      ['Passing Score', '240 (Utah)'],
      ['Score Scale',   '100–300'],
      ['Retake Policy', '30-day minimum wait · No limit on number of attempts'],
      ['Registration',  'ut.nesinc.com — Utah educator testing portal'],
    ]
    ...
    y2R = sectionBar(p2, y2R, 'Test Day Tips — Utah', COL_W, COL2_X)
    const tips = [
      'Utah passing score is 240 — the national benchmark.',
      'Register through ut.nesinc.com — the Utah testing portal.',
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="Utah-FORT-890-Study-Guide.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('Utah PDF error:', err)
