...
export async function GET() {
  try {
    ...
    p1.drawText('FOUNDATIONS OF READING TEST 890', { x: MARGIN, y: PAGE_H - 25, size: 8, font: bold, color: rgb(0.91, 0.71, 0.74) })
    ...
    // ── 890 at a Glance ─────────────────────────────────────────────────────
    y = sectionBar(p1, y, 'FORT 890 — Test at a Glance')

    const glanceCol1W = 96
    const glanceRows: [string, string][] = [
      ['Test Code',     '890 — Foundations of Reading (used by multiple states)'],
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="FORT-890-Study-Guide.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('FORT 890 Study Guide PDF error:', err)
