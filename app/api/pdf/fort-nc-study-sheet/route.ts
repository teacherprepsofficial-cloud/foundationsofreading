...
export async function GET() {
  try {
    ...
    p1.drawText('NORTH CAROLINA — FOUNDATIONS OF READING', { x: MARGIN, y: PAGE_H - 25, size: 8, font: bold, color: rgb(0.91, 0.71, 0.74) })
    p1.drawText('FORT 890 · Study Sheet', { x: MARGIN, y: PAGE_H - 46, size: 17, font: bold, color: WHITE })
    ...
    y = sectionBar(p1, y, 'North Carolina — Test at a Glance')

    const glanceCol1W = 96
    const glanceRows: [string, string][] = [
      ['Test Code',     '890 — Foundations of Reading (North Carolina)'],
      ...
      ['Fee',           '$139'],
      ['Passing Score', '233 (North Carolina)'],
      ['Score Scale',   '100–300'],
      ['Retake Policy', '30-day minimum wait · No limit on number of attempts'],
      ['Registration',  'nc.nesinc.com — North Carolina educator testing portal'],
    ]
    ...
    y2R = sectionBar(p2, y2R, 'Test Day Tips — North Carolina', COL_W, COL2_X)
    const tips = [
      'North Carolina passing score is 233 — lower than the 240 national benchmark.',
      'Register through nc.nesinc.com — the North Carolina testing portal.',
      'Test code is 890. Same content as the NES 190, different registration portal.',
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="NC-FORT-890-Study-Sheet.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('North Carolina PDF error:', err)
