...
export async function GET() {
  try {
    ...
    p1.drawText('MTEL · FOUNDATIONS OF READING', { x: MARGIN, y: PAGE_H - 25, size: 8, font: bold, color: rgb(0.91, 0.71, 0.74) })
    p1.drawText('MTEL 190 · Study Guide', { x: MARGIN, y: PAGE_H - 46, size: 17, font: bold, color: WHITE })
    ...
    // ── MTEL at a Glance ────────────────────────────────────────────────────
    y = sectionBar(p1, y, 'MTEL — Test at a Glance')
    ...
    y2R = sectionBar(p2, y2R, 'Test Day Tips — MTEL', COL_W, COL2_X)
    const tips = [
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="MTEL-190-Foundations-of-Reading-Study-Guide.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('MTEL Study Guide PDF error:', err)
