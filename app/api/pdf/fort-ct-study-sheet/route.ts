...
export async function GET() {
  try {
    ...
    p1.drawText('CONNECTICUT — FOUNDATIONS OF READING', { x: MARGIN, y: PAGE_H - 25, size: 8, font: bold, color: rgb(0.91, 0.71, 0.74) })
    ...
    y = sectionBar(p1, y, 'Connecticut — Test at a Glance')

    const glanceCol1W = 96
    const glanceRows: [string, string][] = [
      ['Test Code',     '890 — Foundations of Reading (Connecticut)'],
      ...
      ['Fee',           '$139'],
      ['Passing Score', '240 (Connecticut)'],
      ['Score Scale',   '100–300'],
      ['Retake Policy', '30-day minimum wait · No limit on number of attempts'],
      ['Registration',  'ct.nesinc.com — Connecticut educator certification testing portal'],
    ]
    ...
    y2R = sectionBar(p2, y2R, 'Test Day Tips — Connecticut', COL_W, COL2_X)
    const tips = [
      'Connecticut passing score is 240 — the national benchmark.',
      'Register through ct.nesinc.com — the Connecticut testing portal.',
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="CT-FORT-890-Study-Sheet.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('Connecticut PDF error:', err)
