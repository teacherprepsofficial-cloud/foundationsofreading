...
export async function GET() {
  try {
    ...
    const glanceRows: [string, string][] = [
      ['Test Code',     '890 — Foundations of Reading (NES in [State] portal)'],
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="FORT-890-Quick-Reference-Guide.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('FORT 890 PDF error:', err)
