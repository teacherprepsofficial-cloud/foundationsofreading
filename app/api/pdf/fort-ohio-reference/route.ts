...
export async function GET() {
  try {
    ...
    p1.drawText('OHIO — FOUNDATIONS OF READING', { x: MARGIN, y: PAGE_H - 25, size: 8, font: bold, color: rgb(0.91, 0.71, 0.74) })
    p1.drawText('OAE 190 · Quick Reference Guide', { x: MARGIN, y: PAGE_H - 46, size: 17, font: bold, color: WHITE })
    ...
    // ── Ohio at a Glance ────────────────────────────────────────────────────
    y = sectionBar(p1, y, 'Ohio — Test at a Glance')
    ...
    y2R = sectionBar(p2, y2R, 'Test Day Tips — Ohio', COL_W, COL2_X)
    const tips = [
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="Ohio-FORT-190-Quick-Reference-Guide.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('Ohio PDF error:', err)
