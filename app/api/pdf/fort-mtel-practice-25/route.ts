...
export async function GET() {
  try {
    ...
    page.drawText('MTEL Foundations of Reading · Free Practice Test', { x: MARGIN, y: PAGE_H - 46, size: 14, font: bold, color: WHITE })
    ...
    for (let qi = 0; qi < questions.length; qi++) {
      ...
      for (const opt of q.options) {
        ...
        for (let li = 0; li < oLines.length; li++) {
          page.drawText(oLines[li], { x: qStartX + 8, y: ty, size: 8.5, font: regular, color: DARK })
          ty -= 11
        }
        ty -= 2
      }
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="MTEL-Foundations-of-Reading-Practice-Test.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('MTEL Practice Test PDF error:', err)
