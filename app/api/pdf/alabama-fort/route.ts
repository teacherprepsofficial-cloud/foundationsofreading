...
export async function GET() {
  try {
    ...
    p1.drawText('ALABAMA — FOUNDATIONS OF READING', { x: MARGIN, y: PAGE_H - 25, size: 8, font: bold, color: rgb(0.91, 0.71, 0.74) })
    ...
    // ── Alabama at a Glance ─────────────────────────────────────────────────
    y = sectionBar(p1, y, 'Alabama — Test at a Glance')

    const glanceCol1W = 96
    const glanceRows: [string, string][] = [
      ['Test Code',     '890 — Foundations of Reading (Alabama uses NES in Alabama)'],
      ...
      ['Fee',           '$139'],
      ['Passing Score', '233 (Alabama) · National benchmark is 240'],
      ['Score Scale',   '100–300'],
      ['Retake Policy', '30-day minimum wait · No limit on number of attempts'],
      ['Registration',  'National Evaluation Series in Alabama page (not the NES 190 page)'],
    ]
    ...
    y2R = sectionBar(p2, y2R, 'Test Day Tips — Alabama', COL_W, COL2_X)
    const tips = [
      'Alabama passing score is 233 — lower than the 240 national benchmark.',
      'Register through the NES in Alabama page, not the standard NES 190 page.',
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="Alabama-FORT-890-Quick-Reference-Guide.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('Alabama PDF error:', err)
