  ...
  try {
    ...
    const drawParagraph = (pg: PDFPage, yPos: number, text: string, size = 8.5): number => {
      ...
      for (const line of lines) {
        if (ty < MARGIN + 40) { /* handled by caller */ }
    ...
    page.drawText('NES 190 · Starter Study Guide', { x: MARGIN, y: PAGE_H - 49, size: 17, font: bold, color: WHITE })
    ...
    // Intro
    y = drawParagraph(page, y, 'This free starter study guide covers Subarea I — Foundations of Reading Development — which accounts for 35% of the NES 190 exam. This is the single largest section and where most of your study time should go.')
    y -= 4
    ...
    y -= 4
    y = drawParagraph(page, y, 'You just covered the most heavily weighted section of the NES 190. But there are three more subareas you need to know:')
    y -= 4
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="NES-190-Starter-Study-Guide.pdf"',
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('Starter study guide PDF error:', err)
