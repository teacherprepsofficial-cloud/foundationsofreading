/**
 * Comprehensive review of PT1 questions for content issues
 * Run: npx tsx scripts/review-pt1-issues.ts
 */
...
async function main() {
  ...
  const questions = await Question.find({ _id: { $in: pt1.questionIds } }).lean() as any[]

  const posMap: Record<string, number> = {}
  pt1.questionIds.forEach((id: mongoose.Types.ObjectId, i: number) => {
    posMap[id.toString()] = i + 1
  })

  console.log(`PT1: ${pt1.name} — ${questions.length} questions\n`)
