/**
 * Comprehensive review of Diagnostic questions for content issues
 * Run: npx tsx scripts/review-diagnostic-issues.ts
 */
...
async function main() {
  ...
  const diagnostic = await PracticeTest.findOne({ examCode: '190', isDiagnostic: true })
  if (!diagnostic) { console.error('Diagnostic not found'); process.exit(1) }

  const questions = await Question.find({ _id: { $in: diagnostic.questionIds } }).lean() as any[]

  const posMap: Record<string, number> = {}
  diagnostic.questionIds.forEach((id: mongoose.Types.ObjectId, i: number) => {
    posMap[id.toString()] = i + 1
  })

  console.log(`Diagnostic: ${diagnostic.name} — ${questions.length} questions\n`)
