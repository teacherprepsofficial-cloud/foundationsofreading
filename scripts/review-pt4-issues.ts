/**
 * Comprehensive review of PT4 questions for content issues
 * Run: npx tsx scripts/review-pt4-issues.ts
 */
...
async function main() {
  ...
  const pt4 = await PracticeTest.findOne({ examCode: '190', testNumber: 4, isDiagnostic: false })
  if (!pt4) { console.error('PT4 not found'); process.exit(1) }

  const questions = await Question.find({ _id: { $in: pt4.questionIds } }).lean() as any[]

  const posMap: Record<string, number> = {}
  pt4.questionIds.forEach((id: mongoose.Types.ObjectId, i: number) => {
    posMap[id.toString()] = i + 1
  })

  console.log(`PT4: ${pt4.name} — ${questions.length} questions\n`)
